...
#define UPDATE()                                                             \
  do {                                                                       \
    T2(1) = S64(1,  66) ^ S64(1,  93);                                        \
    T2(2) = S64(2,  69) ^ S64(2,  84);                                        \
    T2(3) = S64(3,  66) ^ S96(3, 111);                                        \
                                                                             \
    Z(T2(1) ^ T2(2) ^ T2(3));                                                   \
                                                                             \
    T2(1) ^= (S64(1,  91) & S64(1,  92)) ^ S64(2,  78);                       \
    T2(2) ^= (S64(2,  82) & S64(2,  83)) ^ S64(3,  87);                       \
    T2(3) ^= (S96(3, 109) & S96(3, 110)) ^ S64(1,  69);                       \
  } while (0)

#define ROTATE()                                                             \
  do {                                                                       \
    S(1, 3) = S(1, 2); S(1, 2) = S(1, 1); S(1, 1) = T2(3);                    \
    S(2, 3) = S(2, 2); S(2, 2) = S(2, 1); S(2, 1) = T2(1);                    \
    S(3, 4) = S(3, 3); S(3, 3) = S(3, 2); S(3, 2) = S(3, 1); S(3, 1) = T2(2); \
  } while (0)
...
#undef Z
#define Z(w) U32TO8_BIG(output + 4 * i, U8TO32_BIG(input + 4 * i) ^ w)
